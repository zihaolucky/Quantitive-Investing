function [theta, y_test, X_test,error2,AskPrice1,BidPrice1] = linearRegression(Close,AskPrice1,BidPrice1,lambda)
ma1 = 3; ma2 = 5; ma3 = 7;
...
Exp = exp(0.002*(repmat(m,[m,1])-[1:m]'));

cvx_begin quiet
    variable theta(4)
    minimize ( sum(abs(X_train*theta - y_train))/m )  % 一范数形式目标函数
    %minimize ( sum((X_train*theta - y_train).^2)/m )  % 二范数形式目标函数
%     minimize ( sum(abs(X_train*theta - y_train))/m + lambda*sum(theta.^2))  % 加正则项一范数形式目标函数
cvx_end
