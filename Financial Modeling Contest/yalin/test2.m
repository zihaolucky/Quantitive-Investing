function [AnReturn,maxdraw,RBRatio,shp,error2,m]=test2(Close,AskPrice1,BidPrice1,theta,lambda)

[t, y_test, X_test,error2,AskPrice1,BidPrice1] = linearRegression2(Close,AskPrice1,BidPrice1,lambda);
...
MA20=[];
for i=1:15
    MA20(1000*(i-1)+1:1000*i) = X_test(1000*(i-1)+1:1000*i,:)*theta(:,i);
end
a=X_test(1000*15+1:end,:)*theta(:,15);
MA20=[MA20 a'];
MA5 = y_test;
...
for t = LongLen:length(IFdata)
    ...
    if SignalBuy==1
        ...
        if Pos(t-1)==0
            Pos(t)=1;
            %ReturnD(t)=-0.00007*IFdata(t)*scale;
        ...
        if Pos(t-1)==-1
            Pos(t)=1;
            %ReturnD(t)=(-0.00014*IFdata(t))*scale;
    ...
    if SignalSell==1
        ...
        if Pos(t-1)==1
            Pos(t)=-1;
            %Return(t)=(-IFdata(t)*0.00014)*scale;
