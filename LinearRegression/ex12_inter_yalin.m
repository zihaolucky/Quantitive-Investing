function [predict_y2,theta,x]=ex12_inter_yalin(symbol,ma1,ma2,ma3,percent)
%% Instruction
...
% X = data((0.9-percent)*end+1:0.9*end,1:4);
x = data(1:0.9*end+4,1);

X_test = data(0.9*end:end, 1:4);
% X_test = data((0.9-percent)*end+1:0.9*end+4, 1:4);
y = data((0.9-percent)*end+1:0.9*end, 5);
...
% part #2
predict_y2=X_test(end-4:end,:)*theta;
